...
export class MaterialSystem {
  ...
  register(def: MaterialDef): MaterialHandle {
    ...
    if (existing !== undefined) {
      ...
      // Dispose the previously-compiled THREE.Material before dropping the cache entry —
      // unregister() below already does this correctly; re-registering a material by name
      // (e.g. hot-tuning params) was leaking the old compiled material every time.
      const cached = this.threeCache.get(existing);
