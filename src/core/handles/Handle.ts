// Split evenly across the 32 bits available to a bitwise-safe JS integer. The previous
// 20/12 split gave generations only 4096 values before wrapping, which is a small enough
// window that a long-held stale Handle could plausibly alias a freshly-allocated one at the
// same slot, defeating the whole point of the generation check. 65,536 live handles per
// pool and 65,536 reuses before a generation wraps are both comfortably above anything this
// engine allocates in practice (see HandleMap/HandleAllocator call sites).
const INDEX_BITS = 16;
const GEN_BITS = 16;
const INDEX_MASK = (1 << INDEX_BITS) - 1;
const GEN_MASK = (1 << GEN_BITS) - 1;
const MAX_ENTITIES = 1 << INDEX_BITS; // 65,536
...
export class HandleAllocator {
  ...
  /** The current generation for a slot, so callers holding only a raw index (e.g. iterating
   *  HandleMap.forEachEntry) can reconstruct a real, valid Handle instead of casting the
   *  index directly to a Handle (which has generation 0 baked in, correct only by chance). */
...
export class HandleMap<T> {
  ...
  /** Reconstructs a valid Handle for a live slot found via forEachEntry/forEach, where only
   *  the raw array index is available — casting that index directly to a Handle bakes in
   *  generation 0, which is only actually correct if that slot happens to be on its first
   *  generation. */
  handleAt(index: number): Handle | null {
    if (!this.entries[index]) return null;
    return makeHandle(index, this.allocator.generationAt(index));
  }
  ...
  free(handle: Handle): T | null {
    ...
    if (entry.refCount > 1) {
      // Someone else still holds a reference via retain() — don't destroy the underlying
      // resource out from under them, just drop this caller's share. Actual cleanup happens
      // once refCount reaches 1 and free()/release() is called again.
      entry.refCount--;
      return entry.data;
    }
