...
export class SkeletonSystem extends System {
  ...
  activate(instanceHandle: Handle, joints: JointConfig[]): void {
    ...
    for (let i = 0; i < def.boneCount; i++) {
      ...
      // Transform stores Euler angles; instance rest/current pose is a quaternion (e.g. set
      // by startHumanoidRagdoll from the animated pose right before activation) — convert so
      // the physics body doesn't spawn at identity rotation regardless of the actual pose.
      const rot = new Quat(
        instance.localRotX[i], instance.localRotY[i], instance.localRotZ[i], instance.localRotW[i]
      ).toEuler();
    ...
    for (let j = 0; j < joints.length; j++) {
      ...
      instance.jointTypes[jc.childBone] = jc.type;
    }
  ...
  deactivate(instanceHandle: Handle): void {
    ...
    for (let i = 0; i < instance.boneCount; i++) {
      ...
      if (jointHandle !== -1) {
        ...
        instance.jointTypes[i] = JointType.FIXED;
      }
    ...
    for (let a = 0; a < this.activeInstances.length; a++) {
      ...
      for (let i = 0; i < instance.boneCount; i++) {
        if (instance.dirtyFlags[i] & DirtyFlags.MOTOR) {
          const jointHandle = instance.jointHandles[i];
          // Only REVOLUTE/PRISMATIC Rapier joints expose the single-axis motor API used
          // below — SPHERICAL and FIXED joints don't have configureMotorPosition/Velocity
          // at all, so calling them there would throw and take down every active skeleton's
          // update() for the frame. Most humanoid joints (hips/spine/shoulders) are
          // SPHERICAL, so this guard is load-bearing, not defensive padding.
          const jointType = instance.jointTypes[i];
          const motorizable = jointType === JointType.REVOLUTE || jointType === JointType.PRISMATIC;
          if (jointHandle !== -1 && instance.motorEnabled[i] && motorizable) {
