...
export interface RenderContext {
  // Not `| any`: that nullified type-checking for every pass's ctx.renderer usage below.
...
export class ShadowPass implements RenderPass {
  ...
  // Forcing needsUpdate every single frame defeats THREE's selective shadow-map updates for
  // static scenes (every shadow-casting light re-renders its depth map even when nothing
  // moved). Only force it once by default; call markDirty() when something shadow-relevant
  // actually changes (a light or shadow caster moves).
  private dirty = true;
