...
export class ChunkedArchetype {
  ...
  removeEntity(chunkIndex: number, row: number): { swappedEntity: number } | null {
    ...
    // Deliberately NOT splicing out empty chunks here: doing so shifts every later chunk's
    // index down by one, but ChunkedArchetypeStorage.entityLocations (which stores each
    // entity's chunkIndex) has no way to find out about that shift and would keep pointing
    // at the wrong chunk for every entity in a shifted chunk — silent row/column corruption.
    // An empty chunk is simply left in place; findNonFullChunk() already treats count===0 as
    // "not full" and will reuse it on the next addEntity() call.
